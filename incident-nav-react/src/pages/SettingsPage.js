...
import { getUsers, updateUserIndustries, deleteUser, register, updateUserAdminStatus } from '../scripts/auth';
...
export default function SettingsPage() {
    ...
    function handleLlmChange(event) {
        ...
        setApiKeyPlaceHolder(selected?.api_key || '');
    ...
    const handleAddUser = () => {
        if (newUser.username && newUser.email && newUser.password) {
            register(newUser.email, newUser.username, newUser.password).then((response) => {
                if (!response.error) {
    ...
    const handleUserAdminUpdate = (selectedUser) => {
        if (selectedUser === currentUser) {
            return;
        }
        updateUserAdminStatus(selectedUser.id, !selectedUser.isAdmin).then((response) => {
            if (!response.error) {
    ...
    return (
        <div className="p-2 w-full mx-auto">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder={apiKeyPlaceHolder ? apiKeyPlaceHolder :"Enter your API key"}
                            disabled={!selectedLlm}
                        />
                    </div>
            ...
            <div className="bg-light-surface dark:bg-dark-surface p-6 mb-6 shadow-md rounded-lg">
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block font-medium text-gray-700 dark:text-white">Username</label>
                        <input
                            type="text"
                            className="mt-1 p-2 w-full bg-gray-100 dark:bg-neutral-800 rounded-md"
                    ...
                    <div>
                        <label className="block font-medium text-gray-700 dark:text-white">Email</label>
                        <input
                            type="email"
                            className="mt-1 p-2 w-full bg-gray-100 dark:bg-neutral-800 rounded-md"
                            value={newUser.email || ''}
                    ...
                    <div>
                        <label className="block font-medium text-gray-700 dark:text-white">Password</label>
                        <input
                            type="password"
                            className="mt-1 p-2 w-full bg-gray-100 dark:bg-neutral-800 rounded-md"
                    ...
                    >
                        <div className="w-full">
                            ...
                            <div className="flex items-center justify-between mt-4">
                                ...
                                <div
                                    onClick={() => user.id !== currentUser.id && handleUserAdminUpdate(user)}
                                    className={`relative w-14 h-8 rounded-full transition duration-300 
            ${user.id === currentUser.id ? 'bg-gray-500 cursor-not-allowed' : (user.isAdmin ? 'bg-blue-500 cursor-pointer' : 'bg-gray-300 cursor-pointer')}`}
                                >
                                    <div
                                        className={`absolute w-6 h-6 bg-white rounded-full shadow-md top-1 transition-transform duration-300 transform 
                ${user.isAdmin ? 'translate-x-7' : 'translate-x-1'} 
